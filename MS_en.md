...
Code for model fitting, evaluation, and figure generation is available at https://github.com/Akakk1/KAN_Maxent (software MIT License; release tag `ms-submission`). The repository includes the `kanmaxent` Python package, experiment scripts, unit tests, a short smoke-check entry point, pre-computed metrics tables under `results/`, and the rendered main-text and Supporting Information figures. The NCEAS presence-only and presence–absence tables redistributed under `data/nceas/` follow Elith et al. (2020) and remain **CC BY-NC 4.0** (attribution required; non-commercial use only); they are not covered by the MIT licence. Full multi-region re-runs write to a local `outputs/` directory and are not required to recover the published summary statistics.
